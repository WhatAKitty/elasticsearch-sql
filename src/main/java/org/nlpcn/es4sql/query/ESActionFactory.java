...
public class ESActionFactory {
    ...
    public static WriteAction createWrite(Client client, String sql) {
        sql = sql.replaceAll("\n", " ");
        String firstWord = sql.substring(0, sql.indexOf(' '));

        // pre hook get type first
        String type = null;
        int slashIndex = sql.indexOf("/");
        if (slashIndex > -1) {
            // get type first
            boolean flag = false;
        ...
        switch (firstWord.toUpperCase()) {
            case "INSERT":
                ...
                Insert insert = new SqlParser().parseInsert(sqlInsertStatement);
                insert.getTo().setType(type);
                return new InsertWriteAction(client, insert);
            default:
     ...
     * a-z A-Z 0-9
     */
    private static boolean isBit62(int character) {
